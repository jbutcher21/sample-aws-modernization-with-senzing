...
::alert[**Why This Matters**: Without this check, AIs sometimes "invent" fields that seem plausible but don't actually exist in the data. This can cause mapper code to fail or produce incorrect output. Always verify this count matches!]{type="warning"}
...
::alert[**If the count is wrong**: Stop and ask the AI to re-extract fields. Review the schema file manually to identify what was missed or hallucinated.]{type="warning"}
